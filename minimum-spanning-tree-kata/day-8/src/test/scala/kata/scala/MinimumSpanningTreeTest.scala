...
class MinimumSpanningTreeTest extends FlatSpec with Matchers with BeforeAndAfter {
    ...
    it should "have two edges" in {
        ...
        val spinningTree = MinimumSpanningTree.create(graph).get

        spinningTree.tree.toStream should contain allOf (new WeightedEdge(1, 2, 1.0), new WeightedEdge(2, 3, 1.0))
    }
    ...
    it should "have three edges" in {
        ...
        spinningTree.tree.toStream should contain allOf (new WeightedEdge(1, 2, 1.0), new WeightedEdge(2, 3, 1.0), new WeightedEdge(3, 4, 1.0))
    }
