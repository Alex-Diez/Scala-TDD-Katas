...
class GameTest extends FlatSpec with Matchers with BeforeAndAfterEach {
  ...
  private implicit def toTestGame(game: Game): TestGame =
    new TestGame(game)
  ...
  "Game with one spare" should "have bonus of the next roll after spare frame" in {
    game.rollSpare()
    game.roll(3)
    game.rollMany(17, 0)
  ...
  "Game with one strike" should "have bonus of the two next rolls after strike frame" in {
    game.rollStrike()
    game.roll(4)
    game.roll(3)
    game.rollMany(16, 0)
