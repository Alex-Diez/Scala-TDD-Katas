...
class Game {
  ...
  private def score(current: Int, rolls: List[Int], frame: Int): Int = {
    ...
    (frame, rolls) match {
      case (11, _) => current
      case (_, _ :: rest) if isStrike => score(10 + strikeBonus + current, rest, next)
      case (_, _ :: _ :: rest) if isSpare => score(framePoints + spareBonus + current, rest, next)
      case (_, _ :: _ :: rest) => score(framePoints + current, rest, next)
    }
  }
}
