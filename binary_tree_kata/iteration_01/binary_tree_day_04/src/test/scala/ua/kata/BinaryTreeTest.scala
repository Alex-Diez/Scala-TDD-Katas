...
class BinaryTreeTest extends FunSuite with Matchers {
  private val emptyTree = BinaryTree()

  test("tree has size 0 when created") {
    emptyTree should have size 0
  }

  test("tree has size 1 when added item") {
  ...
  test("tree contains all added items") {
  ...
  test("tree has size of number added items") {
    val tree: BinaryTree = emptyTree.add(2).add(1).add(3)
